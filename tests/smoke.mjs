...
try {
  ...
  const migratedFile = readFileSync(join(legacyProj, '.agents', '.memory', 'project.md'), 'utf8')
  assert.match(migratedFile, /^---\nname: project\ntype: project\ndescription: 旧项目情境\n---/m)
  assert.match(migratedFile, /---\n\n# 旧单文件/)
  assert.equal(existsSync(join(legacyProj, '.memory')), false)
  ok('v1 单文件迁移至 .agents/.memory(frontmatter+恰一空行+正文,G3;旧文件已删)')
  mig = migrateLegacyDotMemory(legacyProj)
  assert.equal(mig.moved, false)
  ok('迁移幂等')

  // v2 目录形态:整体原子搬迁
  const dirProj = join(root, 'legacy-dir')
  mkdirSync(join(dirProj, '.git'), { recursive: true })
  mkdirSync(join(dirProj, '.agents'), { recursive: true })
  mkdirSync(join(dirProj, '.memory'), { recursive: true })
  writeFileSync(join(dirProj, '.memory', 'x.md'), '---\ntype: project\ndescription: X\n---\nX\n')
  mig = migrateLegacyDotMemory(dirProj)
  assert.equal(mig.ok, true)
  assert.equal(mig.moved, true)
  assert.ok(existsSync(join(dirProj, '.agents', '.memory', 'x.md')))
  assert.equal(existsSync(join(dirProj, '.memory')), false)
  ok('v2 目录整体原子搬迁至 .agents/.memory(.agents 已存在场景)')
  // 新旧并存:不迁移,留人工合并
  mkdirSync(join(dirProj, '.memory'), { recursive: true })
  writeFileSync(join(dirProj, '.memory', 'y.md'), '---\ntype: project\ndescription: Y\n---\nY\n')
  mig = migrateLegacyDotMemory(dirProj)
  assert.equal(mig.ok, true)
  assert.equal(mig.moved, false)
  assert.ok(existsSync(join(dirProj, '.memory', 'y.md')))
  ok('新旧并存时不迁移(留人工合并)')
  ...
  assert.equal(existsSync(join(proj, '.memory')), false)
  assert.equal(existsSync(join(proj, '.agents', '.memory')), true)
  ok('发现:三层路径与条目(mtime 倒序);旧 proj/.memory 已被惰性整体搬迁')
