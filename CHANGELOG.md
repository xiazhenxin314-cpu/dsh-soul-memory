...
两项用户拍板(2026-09-19):源码布局迁移 + L3 项目层载体迁移。

### Changed

- 源码 `lib/` 整体迁至 `src/`(入口一并迁入,`main=src/soul-memory.mjs`):零构建仓不再使用与「生成物」同名的目录,对齐工作区口径。推翻 2026-09-13 ALPHA_0.5 F10「不移动」结论。挂载面零变化(cordis patch 认包名不认路径),公开导出面逐一不变。
- **L3 项目层载体**:`<projectRoot>/.memory` → `<projectRoot>/.agents/.memory`,项目根不再落 `.memory`。发现层惰性自动搬迁:v1 单文件包 frontmatter 迁入新位(写成功才删旧文件);v2 目录整体原子 rename;新旧并存时不迁移留人工合并;失败均回滚。L1/L2 载体不变。

### Verified

- `npm test` 61 断言全绿(新增:目录整体搬迁、并存不迁移两组 + 发现层搬迁断言并入既有组;59 → 61)。
